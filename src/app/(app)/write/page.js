...
export default function WritePage() {
    ...
    const [activeTab, setActiveTab] = useState('chat'); // Mobile tab state
...
총 ${data.totalCommits}개의 커밋, ${data.repositories.length}개의 레포지토리`;
    ...
    return (
        <div className="flex flex-col h-screen bg-gradient-to-br from-background via-background to-muted/30 overflow-hidden relative font-sans">
            {/* Mobile Tab Bar */}
                    ...
                    <Artifact className="h-full border-none shadow-none rounded-none">
                        <ArtifactHeader className="h-14 shrink-0 bg-background/80 backdrop-blur-sm border-b border-border/50 px-4 lg:rounded-t-xl">
                            ...
                            <ArtifactActions>
                                ...
                                <Button size="sm" onClick={handleSave} disabled={loading} className="font-semibold px-6 bg-primary hover:bg-primary/90 shadow-md transition-all hover:scale-[1.02] rounded-full">
